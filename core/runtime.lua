...
UI.DefaultTheme = {
  ...
  LogoRectOffset = Vector2.new(174, 0),
  LogoRectSize = Vector2.new(987, 987),
  Blur = {
    Enabled = true,
    Size = 4,
  },
    ...
    local function confirmOverwrite(targetName, confirmed)
      if fileExists(targetName) and window and window.Confirm then
        window:Confirm({
          ...
          Content = "Es besteht bereits eine Config mit diesem Namen. M̦chtest du sie ersetzen?",
...
local __NX_STARTED_MODULES = {}

local function nxStartOnce(key, fn)
  if __NX_STARTED_MODULES[key] then
    return true
  end
  __NX_STARTED_MODULES[key] = true
  local ok, err = pcall(fn)
  if not ok then
    __NX_STARTED_MODULES[key] = nil
    error(tostring(err))
  end
  return true
end

function setupCombatSupervisorLoop()
  addConnection("combat.supervisor", RunService.RenderStepped:Connect(function()
    if not RUNNING then return end
    STATE.AimTeamCheck = true
    STATE.AimRequireCenter = true
    STATE.TriggerTeamCheck = true
    ...
    STATE.RageVisibilityCheck = false
    STATE.AimLockStrength = 100
    if enforceAimModeExclusivity then
      local aimModeCount = (STATE.AimLock and 1 or 0) + (STATE.AutoAim and 1 or 0) + (STATE.RageBot and 1 or 0)
      if aimModeCount > 1 then
        enforceAimModeExclusivity()
      end
    end
    updateFOV()
...
function setupRageBotLoop()
  addConnection("combat.ragebot", RunService.RenderStepped:Connect(function(deltaTime)
    if not RUNNING or not Camera then
      resetRageRuntime()
      return
    end
    if not STATE.RageBot or STATE.AimLock or STATE.AutoAim or not isRoundStarted() then
      rageTarget = nil
      rageLockedPlayer = nil
      rageNextShotAt = 0
      restoreRageWeaponLock()
      resetMouseModeState("rage", false)
      return
    end
...
function setupAutoAimLoop()
  addConnection("combat.autoaim", RunService.RenderStepped:Connect(function(deltaTime)
    if not RUNNING or not Camera then
      resetAutoAimRuntime()
      return
    end
    if not STATE.AutoAim or STATE.AimLock or STATE.RageBot or not isRoundStarted() then
      resetAutoAimRuntime()
      return
    end
    ...
    if not autoAimTarget and autoAimLockedPlayer then
      autoAimTarget = getPreferredPartFromPlayer(autoAimLockedPlayer, STATE.AutoAimTargetPart)
    end
    ...
    if targetScreenVisible and modeStrength < 85 then
      local manualDelta = getManualMouseDelta()
      local manualMag = manualDelta.Magnitude
      local manualThreshold = modeStrength >= 100 and 8.5 or 3.0 + math.clamp((100 - modeStrength) * 0.018, 0, 1.8)
      if manualMag > manualThreshold then
        autoAimBreakScore = math.min(2.5, autoAimBreakScore + 0.18)
        mouseStepState.auto = mouseStepState.auto:Lerp(Vector2.new(0, 0), 0.16)
      else
...
function setupSpeedLoop()
  addConnection("movement.speed", RunService.Heartbeat:Connect(function()
    if not RUNNING then return end
    pcall(function()
      local root, hum = getRoot(), getHumanoid()
      if STATE.SpeedEnabled and root and hum and hum.MoveDirection.Magnitude > 0.1 then
        if not speedBV or not speedBV.Parent then
          speedBV = Instance.new("BodyVelocity")
          speedBV.MaxForce = V3(1e5, 0, 1e5)
          speedBV.Parent = root
        end
        speedBV.Velocity = hum.MoveDirection * safeNum(STATE.SpeedValue)
      else
        clearSpeed()
      end
    end)
...
function setupFlyLoop()
  addConnection("movement.fly", RunService.Heartbeat:Connect(function()
    if not RUNNING then return end
    pcall(function()
      local root, hum = getRoot(), getHumanoid()
      if STATE.FlyEnabled and STATE.FlyToggle and root and hum and Camera then
        ...
        flyBV.Velocity = (dir.Magnitude > 0 and dir.Unit or V0) * safeNum(STATE.FlySpeed)
        flyBG.CFrame = CF(root.Position, root.Position + Camera.CFrame.LookVector)
      elseif flyActive or flyBV or flyBG then
        stopFly()
      end
    end)
  end))
...
function setupNoClipLoop()
  addConnection("movement.noclip", RunService.Heartbeat:Connect(function()
    if not RUNNING then return end
    pcall(function()
      if STATE.NoClip then applyNoClip() else restoreNoClip() end
    end)
...
function setupAntiVoidLoop()
  addConnection("movement.antivoid", RunService.Heartbeat:Connect(function()
    if not RUNNING then return end
    pcall(function()
      local root = getRoot()
      if STATE.AntiVoid and root then
        local killY = workspace.FallenPartsDestroyHeight
        if typeof(killY) == "number" and root.Position.Y <= killY + 35 and tick() >= (antiVoidNextAt or 0) then
          antiVoidNextAt = tick() + 0.35
...
function setupAntiHitLoop()
  addConnection("misc.antihit", RunService.Heartbeat:Connect(function()
    if not RUNNING then return end
    pcall(function()
      local root, hum = getRoot(), getHumanoid()
      if STATE.AntiHit and root and hum and not (STATE.FlyEnabled and STATE.FlyToggle) then
...
function setupBunnyHopLoop()
  addConnection("movement.bunnyhop", RunService.Heartbeat:Connect(function()
    if not RUNNING then return end
    pcall(function()
      local hum = getHumanoid()
      if STATE.BunnyHop and hum and hum.MoveDirection.Magnitude > 0 and hum.FloorMaterial ~= Enum.Material.Air then
        hum:ChangeState(Enum.HumanoidStateType.Jumping)
      end
    end)
  end))
...
function setupFOVLoop()
  addConnection("visual.fov", RunService.RenderStepped:Connect(function()
    if RUNNING then
      updateFOV()
    end
  end))
...
local NXRuntime = {
  ...
  StartCombatSupervisorLoop = function() return nxStartOnce("combat.supervisor", setupCombatSupervisorLoop) end,
  StartAimLockLoop = function() return nxStartOnce("combat.aimlock", setupAimLockLoop) end,
  StartAutoAimLoop = function() return nxStartOnce("combat.autoaim", setupAutoAimLoop) end,
  StartMouseAimAssistLoop = function() return nxStartOnce("combat.mouseassist", setupMouseAimAssistLoop) end,
  StartRageBotLoop = function() return nxStartOnce("combat.ragebot", setupRageBotLoop) end,
  StartTriggerLoop = function() return nxStartOnce("combat.trigger", setupTrigger) end,
  StartSilentAimHook = function()
    return nxStartOnce("combat.silent", function()
      task.defer(function()
        task.wait(0.58)
        if tostring(STATE.SilentAimMode or "No Hook") == "No Hook" then
          safeHook(setupGrabber, "Silent Aim No Hook")
        elseif hasSilentHookSupport() then
  ...
  StartESPLoop = function() return nxStartOnce("visual.esp", setupESP) end,
  StartFOVLoop = function() return nxStartOnce("visual.fov", setupFOVLoop) end,
  StartSpeedLoop = function() return nxStartOnce("movement.speed", setupSpeedLoop) end,
  StartFlyLoop = function() return nxStartOnce("movement.fly", setupFlyLoop) end,
  StartNoClipLoop = function() return nxStartOnce("movement.noclip", setupNoClipLoop) end,
  StartInfJumpLoop = function() return nxStartOnce("movement.infjump", setupInfJump) end,
  StartAntiVoidLoop = function() return nxStartOnce("movement.antivoid", setupAntiVoidLoop) end,
