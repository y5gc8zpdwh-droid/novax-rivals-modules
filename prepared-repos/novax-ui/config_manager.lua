...
local function attachConfigManager(UI)
  ...
  function UI:CreateConfigManager(window, stateTable, opts)
    ...
    local tabName = tostring(opts.TabName or "Profile")
    ...
    local function notifyConfig(title, content)
      if window and window.Notify then
        window:Notify({
          Title = tostring(title or "Profil"),
    ...
    if window and window.CreateTab then
      ...
      local listFactory = tab.CreateConfigList
      if listFactory then
        configList = listFactory(tab, {
          Name = "Gespeicherte Profile",
      ...
      local function runConfigAction(action)
        ...
        if not ok then
          ...
          notifyConfig("Profil Fehler", tostring(err))
        end
        busy = false
        return ok
      end
      ...
      local function confirmOverwrite(targetName, confirmed)
        if fileExists(targetName) and window and window.Confirm then
          window:Confirm({
            ...
            Content = "Es besteht bereits ein Profil mit diesem Namen. Moechtest du es ersetzen?",
      ...
      local function saveCurrent()
        ...
        local function doSave()
          runConfigAction(function()
            ...
            notifyConfig(ok and "Profil gespeichert" or "Profil Fehler", ok and ("Gespeichert: " .. tostring(saved)) or tostring(saved))
      ...
      local function createNew()
        runConfigAction(function()
          ...
          notifyConfig(ok and "Profil erstellt" or "Profil Fehler", ok and ("Erstellt: " .. tostring(saved)) or tostring(saved))
          refresh(ok and saved or activeName)
        end)
      end
      ...
      local function loadSelected()
        runConfigAction(function()
          ...
          notifyConfig(ok and "Profil geladen" or "Profil Fehler", ok and ("Geladen: " .. tostring(loaded)) or tostring(loaded))
      ...
      local function deleteSelectedAction()
        ...
        local function deleteSelected()
          runConfigAction(function()
            ...
            notifyConfig(ok and "Profil geloescht" or "Profil Fehler", ok and ("Geloescht: " .. tostring(deleted)) or tostring(deleted))
            refresh()
          end)
        end
        if window and window.Confirm then
          window:Confirm({
            ...
            Content = 'Moechtest du das Profil "' .. tostring(targetName) .. '" wirklich loeschen?',
      ...
      if tab.CreateActionGroup then
        tab:CreateActionGroup({
          Name = "Profil Aktionen",
          Actions = {
            { Name = "Speichern", Style = "Primary", Callback = saveCurrent },
            { Name = "Neu", Style = "Primary", Callback = createNew },
            { Name = "Laden", Style = "Primary", Callback = loadSelected },
            { Name = "Loeschen", Style = "Primary", Callback = deleteSelectedAction },
          },
          AllBlue = true,
        })
      else
        tab:CreateButton({ Name = "Speichern", Callback = saveCurrent })
        tab:CreateButton({ Name = "Neu", Callback = createNew })
        tab:CreateButton({ Name = "Laden", Callback = loadSelected })
        tab:CreateButton({ Name = "Loeschen", Callback = deleteSelectedAction })
      end
